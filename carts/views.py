...
def cart_add(request):
    print("DA????")
    print(request)
    product_id = request.POST.get("product_id")  # from ajax
    ...
    # cart_items_html перерисовываем страницу
    user_cart = get_user_carts(request)
    cart_items_html = render_to_string("carts/includes/included_cart.html", {'carts': user_cart}, request=request)
    response_data = {'cart_items_html': cart_items_html, 'message': 'Tovar dobavlen v korziny'}
